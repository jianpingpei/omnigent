...
def _close_quietly(resource: _Closable, *, description: str) -> None:
    try:
        resource.close()
    except Exception as exc:
        logger.warning("Could not close %s: %s", description, _safe_provider_error(exc))
...
class OpenSandboxLauncher(ExecModelHostLauncher):
    ...
    def _remember(self, client: SandboxSync) -> SandboxSync:
        ...
        if existing is not client:
            _close_quietly(client, description=f"duplicate sandbox client {client.id}")
        return existing
    ...
    def prepare(self) -> None:
        ...
        try:
            ...
            if manager is not None:
                _close_quietly(manager, description="OpenSandbox preflight manager")
    ...
    @staticmethod
    def _destroy_after_failed_create(sandbox: SandboxSync) -> None:
        try:
            sandbox.kill()
        except Exception as exc:
            logger.warning(
    ...
    def run(self, sandbox_id: str, command: str, *, check: bool = True) -> RemoteCommandResult:
        ...
        try:
            execution = client.commands.run(command, opts=RunCommandOpts())
            ...
            synthetic_error = returncode is None and execution_error is not None
    ...
    def terminate(self, sandbox_id: str) -> None:
        ...
        try:
            ...
            if manager is not None:
                _close_quietly(manager, description="OpenSandbox termination manager")
            if client is not None:
                _close_quietly(client, description=f"terminated OpenSandbox client {sandbox_id}")
    ...
    def _wait_for_terminal(
        ...
        *,
        ...
        try:
            ...
            if owns_manager:
                _close_quietly(active_manager, description="OpenSandbox delete poll manager")
    ...
    def is_running(self, sandbox_id: str) -> bool | None:
        ...
        try:
            ...
            if manager is not None:
                _close_quietly(manager, description="OpenSandbox status manager")
        return _state_name(info.status.state) == "running"
